      ...
      <div className="flex flex-col gap-2">
        ...
        <PostStats post={post} userId={user.id} />
          ...
          <div className="flex w-full px-2 ">
            <h3 className="text-gray-300 text-sm font-semibold">{post?.creator?.username}</h3> 
            ...
            <div className="text-gray-400 text-sm text-start pl-2 ">
              ...
              <div className="flex gap-4 pt-1">
                ...
                className="bg-gray-800 text-[13px] border rounded-full px-4" />
                <button><BsFillSendFill /></button>
              </div>
              <p className="pt-2">{convertCreatedAtToRelativeTime(post?.$createdAt)}</p>
            </div>
      </div>
    </div>
  );
